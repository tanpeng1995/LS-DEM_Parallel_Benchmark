...
void reassign_particles(int rank, string indir){
  if(USED > 1){
    if(rank < USED){
      ...
      for(int i = 0; i < num_grains; ++i){
        if(belongToThisRank[i] == 1){
          //belongToThisRank[i] == 1 means #grainsWorld[i] belongs to this rank
          find_pos(grainsWorld[i].getPosition(), _idx, _idy, _idz);
          //_idx = int((grainsWorld[i].getPosition()(0)-minX)/CUTOFF);
          //_idy = int((grainsWorld[i].getPosition()(1)-minY)/CUTOFF);
          //_idz = int((grainsWorld[i].getPosition()(2)-minZ)/CUTOFF);
