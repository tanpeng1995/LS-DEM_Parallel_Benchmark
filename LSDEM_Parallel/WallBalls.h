...
public:
	...
	void findWallForceMoment(std::unique_ptr<Grain3d> & grain, Vector3d & grainForce, Vector3d & grainMoment, Vector6d & stressVoigt) {
	...
	void computeInternalForceOnTopLayer(std::unique_ptr<WallCap> & wallCap, double dt) const {
		...
		for (int i = 0; i < _nangular; i++) {
			...
			for (int n = 0; n < 6; n++) {
				...
				Vector3d force = _kmn*(dist-_l0)*normal;
				/* shear force */
