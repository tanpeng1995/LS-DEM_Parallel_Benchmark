...
void init_simulation(int rank, int num_procs, string indir, FILE * wallfile, FILE * wallfile2){
  ...
  determine_proc(num_grains, num_procs-1, USED, dimX, dimY, dimZ);
  ...
  int ranks_boundary[USED+1]; // add one to handle boundary
  for(int i = 0; i < USED+1; ++i) ranks_boundary[i] = i;
  MPI_Group_incl(world_group, USED+1, ranks_boundary, &boundary_group);
  MPI_Comm_create(MPI_COMM_WORLD, boundary_group, &BOUNDARY_COMM);
  if(rank == 0) cout<<"finish building BOUNDARY_COMM..."<<endl;
  ...
  const int items4 = 3;
  int lengths4[3] = {1, 4, 3};
  MPI_Datatype types4[3] = {MPI_INT, MPI_DOUBLE, MPI_DOUBLE};
  MPI_Aint offsets4[3];
  offsets4[0] = offsetof(boundary_data_send, _id);
  offsets4[1] = offsetof(boundary_data_send, _quat);
  offsets4[2] = offsetof(boundary_data_send, _position);
  MPI_Type_create_struct(items4, lengths4, offsets4, types4, &BOUNDARY_SEND);
  MPI_Type_commit(&BOUNDARY_SEND);

  const int items5 = 3;
  int lengths5[3] = {1, 3, 3};
  MPI_Datatype types5[3] = {MPI_INT, MPI_DOUBLE, MPI_DOUBLE};
  MPI_Aint offsets5[3];
  offsets5[0] = offsetof(boundary_data_recv, _id);
  offsets5[1] = offsetof(boundary_data_recv, _force);
  offsets5[2] = offsetof(boundary_data_recv, _moment);
  MPI_Type_create_struct(items5, lengths5, offsets5, types5, &BOUNDARY_RECV);
  MPI_Type_commit(&BOUNDARY_RECV);
  ...
  if(rank <= USED){
    ...
    if(rank < USED){
      ...
      totalBoundaryDataSend.resize(USED);
      totalBoundaryDataRecv.resize(USED);
    ...
    MPI_Allreduce(MPI_IN_PLACE, &minMass, 1, MPI_DOUBLE, MPI_MIN, BOUNDARY_COMM);
    MPI_Allreduce(MPI_IN_PLACE, &maxMass, 1, MPI_DOUBLE, MPI_MAX, BOUNDARY_COMM);
    MPI_Allreduce(MPI_IN_PLACE, &maxR, 1, MPI_DOUBLE, MPI_MAX, BOUNDARY_COMM);
    MPI_Allreduce(MPI_IN_PLACE, &avgRadius, 1, MPI_DOUBLE, MPI_SUM, BOUNDARY_COMM);
    MPI_Allreduce(MPI_IN_PLACE, &totalMass, 1, MPI_DOUBLE, MPI_SUM, BOUNDARY_COMM);
