...
public:
  ...
  void takeTimeStepStrainControl2(const Vector3d & disp, double dt){
    double _dd = -_normalGlobal.dot(_center - _height/2.*_normalGlobal);
    double   d = _ramPos.dot(_normalGlobal) + _dd;
    Vector3d ramForce = Vector3d(0.,0.,-1.)*d*_kn;
    Vector3d ramPtc = _ramPos - _center;
    Vector3d ramMoment = ramPtc.cross(ramForce);
    _force += ramForce;
    _moment += ramMoment;
    _velocity = 1./(1.+_damping*dt/2.)*((1.-_damping*dt/2.)*_velocity+dt*_force/_mass);
    Vector3d principleMoment = _rotMatrix.transpose()*_moment; // global to local
    Vector3d omegaN = _omega;
    Vector3d momentInertia;
    momentInertia << _mass*(3*_radius*_radius+_height*_height)/12., _mass*(3*_radius*_radius+_height*_height)/12, _mass*_radius*_radius/2.;

    for (int i = 0; i < 5; i++) {
  		_dOmega(0) = (principleMoment(0) + omegaN(1)*omegaN(2)*(momentInertia(1)-momentInertia(2)) - _damping*momentInertia(0)*omegaN(0) )*dt/momentInertia(0);
  		_dOmega(1) = (principleMoment(1) + omegaN(2)*omegaN(0)*(momentInertia(2)-momentInertia(0)) - _damping*momentInertia(1)*omegaN(1) )*dt/momentInertia(1);
  		_dOmega(2) = (principleMoment(2) + omegaN(0)*omegaN(1)*(momentInertia(0)-momentInertia(1)) - _damping*momentInertia(2)*omegaN(2) )*dt/momentInertia(2);
  		omegaN = _omega + _dOmega/2.;
  	}
    if(isinf(_omega(0)) || isnan(_omega(0)) || isinf(_omega(1)) || isnan(_omega(1)) || isinf(_omega(2)) || isnan(_omega(2))){
      cout << "Issue in WallCap in strain control goes into infinity..." << endl;
    ...
    _position = _center + _height/2.*_normalGlobal;
