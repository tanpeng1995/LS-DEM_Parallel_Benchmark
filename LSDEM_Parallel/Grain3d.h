...
public:
  ...
  void findInterparticleForceMoment(std::unique_ptr<Grain3d> & other, const double & dt, Vector6d & stressVoigt, vector<Vector6d> & posChain, vector<Vector3d> & forceChain, bool output){
    ...
    for(int i = 0; i < _pointList.size(); i+=resolution){
      ...
      if(ptOtherCM.squaredNorm()<other->getRsq()){
        ...
        if(other->getLset().findPenetration(ptOtherLset, penetration, normal)){
          ...
          if(_nodeContact[i] != -1 && _nodeContact[i] != other->_id){
          ...
          _nodeContact[i] = other->_id; //contactId of the ith node
        ...
        else if(_nodeContact[i] == other->_id){
          _nodeContact[i] = -1;
          _nodeShears[i]  << 0., 0., 0.;
    ...
    if(ncontacts > 0 && output){
      #pragma omp critical
      {
        _CN += 1;
        other->_CN += 1;
        Vector6d bv;
        bv << _position(0), _position(1), _position(2), other->_position(0), other->_position(1), other->_position(2);
        posChain.push_back(bv);
        forceChain.push_back(force/(double)ncontacts);
      }
    }
  }
  ...
  const int & getCN() const{
    return _CN;
  }
  void changeCN(int CN) {
    _CN = CN;
  }
