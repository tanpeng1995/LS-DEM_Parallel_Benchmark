...
public:
	...
	void findWallForceMoment(Grain3d & grain, double dt, Vector6d & stressVoigt) {
		Vector3d grainPos = grain.getPosition();
		Vector3d grainForce(0.,0.,0.);
		Vector3d branchVec, normal, v, Fn;
		double penetration = 0.;
		for (int ball_id = 0; ball_id < _nballs; ++ball_id) {
			branchVec = grainPos - _positions[ball_id];
			penetration = branchVec.norm() - _ballRadius - grain.getRadius();
			if(penetration < 0.){
				normal = branchVec/branchVec.norm();
				v = _velocities[ball_id]-grain.getVelocity();
	...
	void computeInternalForceOnTopLayer(std::shared_ptr<WallCap> wallCap, double dt) const {
		double GamaN = -2*sqrt(_kmn*_ballMass/2.)*log(0.6)/sqrt(M_PI*M_PI+log(0.6)*log(0.6));
		for (int i = 0; i < _nangular; i++) {
			int ball_id = i*_nvert + _nvert-1;
	...
	void rotateTopLayer(std::shared_ptr<WallCap> wallCap) {
	...
	vector<int> getBallsRank() {
		...
		for(int i = 0; i < _nballs; ++i){
			if(_touched[i]) ballsRank.push_back(i);
			_touched[i] = false;
		}
		return ballsRank;
	}
