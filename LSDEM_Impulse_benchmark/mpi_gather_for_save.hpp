#ifndef MPI_GATHER_FOR_SAVE_HPP_
#define MPI_GATHER_FOR_SAVE_HPP_
#include "mpi_helper_function.hpp"

void gather_for_save(int rank, int step, string outdir, FILE* posfile, FILE* rotfile){
  if(rank < USED){
    if(rank > 0){
      MPI_Request request;
...
#endif /*MPI_GATHER_FOR_SAVE_HPP_*/
