...
inline bool ghost_bin(int idx, int idy, int idz, int rank){
  ...
  return (idz == pz*(binsInDimZ-2)-1 && idy >= py*(binsInDimY-2)-1 && idy <= (py+1)*(binsInDimY-2)+1
          && idx >= px*(binsInDimX-2)-1 && idx <= (px+1)*(binsInDimX-2)+1) || /*bottom*/
         (idz == (pz+1)*(binsInDimZ-2)+1 && idy >= py*(binsInDimY-2)-1 && idy <= (py+1)*(binsInDimY-2)+1
          && idx >= px*(binsInDimX-2)-1 && idx <= (px+1)*(binsInDimX-2)+1) || /*upper*/
         (idz >= pz*(binsInDimZ-2)-1 && idz <= (pz+1)*(binsInDimZ-2)+1 && idy == py*(binsInDimY-2)-1
          && idx >= px*(binsInDimX-2)-1 && idx <= (px+1)*(binsInDimX-2)+1) || /*front*/
         (idz >= pz*(binsInDimZ-2)-1 && idz <= (pz+1)*(binsInDimZ-2)+1 && idy == (py+1)*(binsInDimY-2)+1
          && idx >= px*(binsInDimX-2)-1 && idx <= (px+1)*(binsInDimX-2)+1) || /*back*/
         (idz >= pz*(binsInDimZ-2)-1 && idz <= (pz+1)*(binsInDimZ-2)+1 && idy >= py*(binsInDimY-2)-1
          && idy <= (py+1)*(binsInDimY-2)+1 && idx == px*(binsInDimX-2)-1) || /*left*/
         (idz >= pz*(binsInDimZ-2)-1 && idz <= (pz+1)*(binsInDimZ-2)+1 && idy >= py*(binsInDimY-2)-1
          && idy <= (py+1)*(binsInDimY-2)+1 && idx == px*(binsInDimX-2)-1) ; /*right*/
}
