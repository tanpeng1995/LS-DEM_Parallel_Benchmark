...
void simulate_one_step(
  ...
  if(rank < USED){
    ...
    vector<int> realGrain(USED,0);
    vector<int> ghostGrain(USED,0);
    int realGrainThisRank = 0;
    int ghostGrainThisRank = 0;
      ...
      */
      ...
      if(belongToThisRank[i]==1){
        ...
        //grainsWorld[i].applyAcceleration(Vector3d(0.,0.,-10/scalingFactor));
	      grainsWorld[i].applyAcceleration(Vector3d(rand_real(gen), rand_real(gen), rand_real(gen))*10./scalingFactor);
    ...
    //MPI_Allgather
    MPI_Allgather(&realGrainThisRank, 1, MPI_INT, realGrain.data(), 1, MPI_INT, CART_COMM);
    MPI_Allgather(&ghostGrainThisRank, 1, MPI_INT, ghostGrain.data(), 1, MPI_INT, CART_COMM);
    //MPI_Allreduce
    MPI_Allreduce(MPI_IN_PLACE, &realGrainThisRank, 1, MPI_INT, MPI_SUM, CART_COMM);
    MPI_Allreduce(MPI_IN_PLACE, &ghostGrainThisRank, 1, MPI_INT, MPI_SUM, CART_COMM);
    if(step % 5 == 0 && rank == 0){
      for(int i = 0; i < USED; ++i){
        //cout<<"rank "<<i<<" real grain: "<<realGrain[i]<<" ghost grain: "<<ghostGrain[i]<<endl;
      }
      //cout<<"All real grain: "<<realGrainThisRank<<". All ghost grain: "<<ghostGrainThisRank<<endl;
    }
