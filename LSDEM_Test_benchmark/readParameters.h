...
Grain3d generateGrainFromFile(const Vector3d & position, const Vector4d & quat, const string & morphfile, int grainidx) {
  ...
  //             mass  position  velocity   momentInertia   quat  omega  cmLset  refPointList  radius  lset  kn  ks  mu  id
	return Grain3d(mass, position, velocity, momentOfInertia, quat, omega, cmLset, refPointList, radius, lset, kn, ks, mu, grainidx);
}
...
void readParametersTrueUnit(string parametersdir){
  ...
  scalingFactor = atof(parameters_line.c_str());
	scalingFactor = scalingFactor*1e-1;
	//cout<<"scalingFactor = "<<scalingFactor<<" (m/pixel)"<<endl;
  ...
  grainDensity = atof(parameters_line.c_str());
	//cout<<"grainDensity = "<<grainDensity<<" (Kg/m^3)"<<endl;
	grainDensity = grainDensity*scalingFactor*scalingFactor*scalingFactor;
	//cout<<"grainDensity = "<<grainDensity<<" (Kg/pixel^3)"<<endl;
  ...
  gDamping = atof(parameters_line.c_str());
	//cout<<"gDamping = "<<gDamping<<" (/seconds)"<<endl;

  getline(parametersFile, parameters_line,':');
  getline(parametersFile, parameters_line);
  worldWidth = atof(parameters_line.c_str());
	//cout<<"worldWidth = "<<worldWidth<<" (m)"<<endl;
	//worldWidth = worldWidth/scalingFactor;
	//cout<<"worldWidth = "<<worldWidth<<" (pixel)"<<endl;

  getline(parametersFile, parameters_line,':');
  getline(parametersFile, parameters_line);
  worldLength = atof(parameters_line.c_str());
	//cout<<"worldLength = "<<worldLength<<" (m)"<<endl;
	//worldLength = worldLength/scalingFactor;
	//cout<<"worldLength = "<<worldLength<<" (pixel)"<<endl;

  getline(parametersFile, parameters_line,':');
  getline(parametersFile, parameters_line);
  worldHeight = atof(parameters_line.c_str());
	//cout<<"worldHeight = "<<worldHeight<<" (m)"<<endl;
	//worldHeight = worldHeight/scalingFactor;
	//cout<<"worldHeight = "<<worldHeight<<" (pixel)"<<endl;
