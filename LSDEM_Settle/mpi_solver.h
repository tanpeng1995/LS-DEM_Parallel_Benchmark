...
void simulate_one_step(
  ...
  if(rank < USED){
    ...
    for(int i = 0; i < num_grains; ++i){
      ...
      */
      ...
      if(belongToThisRank[i]==1){
        ...
        grainsWorld[i]->applyAcceleration(Vector3d(0.,0.,gravity));
    ...
    if(topPlane->getHeight() >= endHeight){
      topPlane->takeTimeStepStrainControl(dHeight);
      wallCylinder->reduceRadius(-dHeight);
    }
