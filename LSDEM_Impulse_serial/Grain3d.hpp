...
public:
  ...
  void findInterGrainContact(const std::unique_ptr<Grain3d> & other, vector<ContactInfo> & contactList, vector<int> & otherContactId, vector<int> & contactId );
  ...
  void addVelocityCompression(const Vector3d & vel, Vector3d normal, int count);
...
bool Grain3d::bCircleCheck(const std::unique_ptr<Grain3d> & other) const{
  Vector3d d = other->getPosition()-_position;
  double rsum = other->getRadius()+_radius;
  return d.squaredNorm() < rsum*rsum;
}

void Grain3d::findInterGrainContact(const std::unique_ptr<Grain3d> & other, vector<ContactInfo> & contactList, vector<int> & otherContactId, vector<int> & contactId){
  ...
  for(int i = 0; i < contactNumbers.size(); ++i){
    ...
    Vector3d n   = contactNormals[i]/contactNormals[i].norm();
...
void Grain3d::addVelocityCompression(const Vector3d & vel, Vector3d normal, int count){
  _velocity += vel;
  //_velocity -= 0.001 * _velocity.dot(normal) * normal;
}
...
static std::vector<std::unique_ptr<Grain3d>> grainsWorld; //each process have a copy of all grains
...
public:
  ...
  void computeRelVelocityWithTopography(){
    _velocity = grainsWorld[_master]->getVelocity() + grainsWorld[_master]->getOmegaGlobal().cross(_masterR);
    _normalV = _velocity.dot(_normal);
  }

  void computeRelVelocityWithTopographyThreshold(){
    _velocity = grainsWorld[_master]->getVelocity() + grainsWorld[_master]->getOmegaGlobal().cross(_masterR);
    _normalV = _velocity.dot(_normal);
    if( abs(_normalV) < threshold ) { _normalV = 0.; }
  }
