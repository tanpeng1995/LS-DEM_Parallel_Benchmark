...
const int SIZE  = 250; //number of grains per bins
...
//VERY IMPORTANT: when a struct contains Vector4d for mpi communication, better to use array instead.
struct data_t{
  int _id; double _quat[4]; Vector3d _position; Vector3d _velocity; Vector3d _omega;
  data_t(){
  ...
  data_t(const int& id, const Vector4d& quat, const Vector3d & position, const Vector3d & velocity, const Vector3d & omega):
    _id(id), _position(position), _velocity(velocity), _omega(omega){
...
struct complete_data_t{
  int _id; int _npoints; double _quat[4]; Vector3d _position; Vector3d _velocity; Vector3d _omega;
  complete_data_t(){
...
struct output_data{
  int _id; double _quat[4]; Vector3d _position;
  output_data(){
...
struct MPIBalls{
  ...
  int _id;
  Vector3d _force;
  MPIBalls(){_rank = 0; _id = 0; _force << 0.,0.,0.;}
  MPIBalls(int rank, int id, Vector3d force): _rank(rank), _id(id), _force(force){}
  ~MPIBalls(){_rank = 0; _id = 0; _force << 0.,0.,0.;}
  MPIBalls(const MPIBalls & other){_rank = other._rank; _id = other._id; _force = other._force;}
  MPIBalls & operator=(const MPIBalls & other){_rank = other._rank; _id = other._id; _force = other._force; return *this;}
};

//global variables
bool restart;
bool massScaling;
double massScalingCoefficient;
bool outputForceChain;
bool outputBallRank;
string testName;
string InitState;
string outdirName;
std::vector<int> loadingStages;
int tmax;
int tConsolidate;
double timeFac;
int outputFrequency;
...
std::unique_ptr<WallBalls> wallBalls; //specified to extra_proc if exists
std::unique_ptr<WallBalls> externalWall;
std::vector<std::unique_ptr<WallPlane>> wallPlanes;
std::unique_ptr<WallPlane> wallPlane;
std::unique_ptr<WallCap> wallCap;
