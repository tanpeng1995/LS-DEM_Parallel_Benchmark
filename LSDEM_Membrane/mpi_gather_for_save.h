...
//use for debug
void print(const output_data & t){
 cout<<"id is: "<<t._id<<endl;
 cout<<"quat is: "<<endl;
 for(size_t i = 0; i < 4; ++i) cout<<t._quat[i]<<endl;
 cout<<"position is: "<<endl;
 cout<<t._position<<endl<<endl;
}
...
void gather_for_save(int rank, int step, string outdir, string indir, FILE* posfile, FILE* rotfile, FILE* wallfile, FILE* wallfile2, FILE * stressFile){
 if(rank < USED){
   if(rank > 0){
     ...
     MPI_Isend(&numOfSendInfo,1,MPI_INT,0,rank,USING_COMM,&request);
     if(numOfSendInfo != 0){
       MPI_Send(sendInfo.data(),numOfSendInfo,OUTPUT_DATA,0,rank,USING_COMM);
     }
     ...
     for(int _rank = 1; _rank < USED; ++_rank){
       MPI_Status r_status;
       int numOfRecvInfo = 0;
       MPI_Recv(&numOfRecvInfo,1,MPI_INT,_rank,_rank,USING_COMM,&r_status);
       if(numOfRecvInfo != 0){
         MPI_Status delivery_status;
         vector<output_data> recvInfo;
         recvInfo.resize(numOfRecvInfo);
         MPI_Recv(recvInfo.data(),numOfRecvInfo,OUTPUT_DATA,_rank,_rank,USING_COMM,&delivery_status);
